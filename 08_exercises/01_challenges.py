...
def flip_booleans(bools):
    ...
    for bool in bools:
        if bool == True:
           swapped_values.append(False)
        elif bool == False:
            swapped_values.append(True)
    return swapped_values 
    # swapped_bools = [bool == False for bool in bools if bool == True]
    # return swapped_bools
...
def translate_key(student, key_to_change, translation):
    ...
    for key in student:
        if key == key_to_change:
           new_student[translation] = student[key]
        else:
...
def get_pug_owners(dogs):
    return [dog['owner'] for dog in dogs if dog['breed'] == 'Pug']
...
def pluralise_keys(dictionary):
    ...
    for k,v in dictionary.items():
        if isinstance(v, list):
            new_dictionary[k + 's'] = dictionary[k]
        else: 
...
def get_word_lengths(string):
    ...
    words = string.split()
    for word in words:
        length_of_string.append(len(word))
