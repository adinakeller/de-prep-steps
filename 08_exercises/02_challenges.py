    ...
    if mobile_number.startswith('07'):
        if len(mobile_number) == 11:
            if mobile_number.isdigit():
                return True
    elif mobile_number.startswith('+447'):
        if len(mobile_number) == 13:
            if mobile_number[3:].isdigit():
                return True
    elif mobile_number.startswith('00447'):
...
def get_williams(names):
    ...
    for name in names:
        ...
        if first_last[1] == 'Williams':
            surname_William.append(name)
    return surname_William
...
def find_wrong_way_fruit(orchard):
    for fruit in orchard:
        if orchard.count(fruit) == 1:
            diff_fruit = fruit
            return orchard.index(diff_fruit)
...
def dna_pairs(dna_string):
    ...
    GCTA = dna_string.upper()
    for item in GCTA:
        if item == 'G':
           list_dna_pairs.append('GC')
        elif item == 'C':
            list_dna_pairs.append('CG') 
        elif item == 'T':
            list_dna_pairs.append('TA')
        elif item == 'A':
            list_dna_pairs.append('AT')
    return list_dna_pairs

    # if 'G' in GCTA:
    #     list_dna_pairs.append('GC') - doesnt work bc doesnt stops when first if is carried out.
    
# make all upper
# give variables to pairs
# if this dna return this pair
